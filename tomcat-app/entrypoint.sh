...
export DB_SCHEMA_VERSION=$(mysql -s -N -hmysql -uroot -p${MYSQL_ROOT_PASSWORD} -e "select schema_version from identityiq.spt_database_version;")
if [ -z "$DB_SCHEMA_VERSION" ]
then
	echo "No schema present, creating IIQ schema in DB" 
	# create database schema
	mysql -uroot -p${MYSQL_ROOT_PASSWORD} -hmysql < /opt/tomcat/webapps/identityiq/WEB-INF/database/create_identityiq_tables-${IIQ_VERSION}.mysql
...
export DB_SPADMIN_PRESENT=$(mysql -s -N -hmysql -uroot -p${MYSQL_ROOT_PASSWORD} -e "select name from identityiq.spt_identity where name='spadmin';")
